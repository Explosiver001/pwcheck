...
long getValue(char *buffer){
    ...
    for(int i = 0; (i < lenght) && (i < 10); i++){
        ...
        if(isDigit(buffer[i])){
            /*multiply value by ten and add new digit as unit*/
            value = value * 10 + buffer[i] - '0'; 
        }
        /*in case of different character than digit*/
        else return -1;
    }
    return value;
}
...
struct inputArguments_t inputCheck(int argc, char *argv[]){

    struct inputArguments_t output;
    output.level = 0;
    output.param = 0; 
    output.useStats = 0;
    ...
    /*if parametr is missing and level haven't been set as optional*/
    if((output.param == 0) && (output.level != 0) && (optionalArgs == 0)){
        output.param = -1;
    }
    /*in case of choosing arguments as optional*/
    else{
        if(output.param == 0){
            output.param = 1;
        } 
...
int passwordProcess1(struct inputArguments_t inputArguments, struct statistics_t stats){
...
int main(int argc, char *argv[]){
    ...
    inputArguments = inputCheck(argc, argv);
    ...
    return passwordProcess1(inputArguments, stats);
}